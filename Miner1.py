...
if sys.argv.__len__() != 2:
    print(colorama.Fore.WHITE + "[" + colorama.Fore.RED + "ERROR" + colorama.Fore.WHITE + "] " + colorama.Fore.RED + "Dm Izikut")
    os.system("pause")
    exit()
else:
    if not re.match('^(bc1|[13])[a-zA-HJ-NP-Z0-9]{25,39}$', sys.argv[1]):
        ...
        os.system("pause")
        exit()
if requests.get("https:ecauto.github.ioKittenMiner.txt").text == "false":
    ...
    os.system("pause")
    exit()
btc_currency = int(requests.get("https:api.coinbase.comv2pricesspot?currency=USD").json()["data"]["amount"].split(".", 1)[0])
nb_essais = 100
def random_btc_value_gen(random_pub_adress):
    if random.randint(0, 2000000) == 0:
